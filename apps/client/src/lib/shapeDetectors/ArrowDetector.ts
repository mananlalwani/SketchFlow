...
export class ArrowDetector implements ShapeDetector {
  ...
  private analyzeVShape(points: Point[]): { apexIndex: number; wing1: Point; wing2: Point; symmetry: number; angleQuality: number } | null {
    const n = points.length;
    if (n < 3) return null;
  ...
  private analyzeDirectionChanges(points: Point[], mainDirection: ReturnType<typeof createVector>): { hasArrowPattern: boolean; quality: number } {
    if (points.length < 2) {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      if (angle > Math.PI / 6) { // More than 30 degrees deviation
        divergenceCount++;
  ...
  private calculateArrowConfidence(shaftStraightness: number, vSymmetry: number, angleQuality: number): number {
