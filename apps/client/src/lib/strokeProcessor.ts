...
import { Point, BoundingBox, getBoundingBox, resamplePath, smoothPath, calculateTotalPathLength, isClosedPath } from './geometry';
...
export function processStroke(
  ...
  const adaptiveTolerance = Math.max(0.1, Math.min(opts.simplificationTolerance, Math.min(initialBbox.width, initialBbox.height) * 0.01));
  ...
  // Check closure
  const diagonal = Math.sqrt(finalBbox.width * finalBbox.width + finalBbox.height * finalBbox.height);
  ...
  for (let i = 1; i < points.length; i++) {
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      if (previousAngle !== null) {
        ...
        if (angleDiff > Math.PI / 12) { // 15 degrees (was 30)
          directionChanges++;
        }
      }
...
function findCorners(points: Point[], angleThreshold: number = Math.PI * 0.6): Point[] { // 108° deviation from straight (72° min angle)
  ...
  console.log(`findCorners: Processing ${points.length} points, minCornerAngle=${minCornerAngleDeg}°`);
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    const tooCloseToExisting = corners.some(corner => 
      Math.sqrt((corner.x - p2.x) ** 2 + (corner.y - p2.y) ** 2) < minCornerSeparation
    );
    ...
    if (len1 > 0 && len2 > 0) {
      ...
      if (angle > minCornerAngle) {
        corners.push(p2);
        if (corners.length <= 10) { // Only log first 10 to avoid spam
          console.log(`  Corner ${corners.length}: angle=${(angle * 180 / Math.PI).toFixed(1)}° at (${p2.x.toFixed(1)}, ${p2.y.toFixed(1)})`);
        }
      }
    }
  }
...
function determineDominantDirection(
  ...
  console.log(`Direction analysis: horizontal=${horizontalMovement.toFixed(1)}, vertical=${verticalMovement.toFixed(1)}, circularIndicator=${circularIndicator.toFixed(1)}, circularRatio=${circularRatio.toFixed(2)}`);
  
  if (circularRatio > 2) { // Lowered from 5 to 2
  ...
  for (const point of points) {
    ...
    for (const otherPoint of points) {
      const distance = Math.sqrt(
        (point.x - otherPoint.x) ** 2 + (reflectedY - otherPoint.y) ** 2
      );
...
function testVerticalSymmetry(points: Point[], bbox: BoundingBox): number {
  ...
  for (const point of points) {
    ...
    for (const otherPoint of points) {
      const distance = Math.sqrt(
        (reflectedX - otherPoint.x) ** 2 + (point.y - otherPoint.y) ** 2
      );
      closestDistance = Math.min(closestDistance, distance);
    }
