...
export class EllipseDetector implements ShapeDetector {
  ...
  detect(stroke: ProcessedStroke, thresholds: DetectionThresholds): DetectionResult | null {
    ...
    console.log(
      `Ellipse: Analysis - corners: ${analysis.cornerCount}, hasSharp: ${analysis.hasSharpCorners}, direction: ${analysis.dominantDirection}, symmetry: ${analysis.symmetry.toFixed(3)}, straightRatio: ${analysis.straightSegmentRatio.toFixed(3)}`,
    );

    // Reject shapes with too many straight segments - ellipses should be curved
    if (analysis.straightSegmentRatio > 0.5) {
      console.log(
        `Ellipse: REJECT - too many straight segments: ${analysis.straightSegmentRatio.toFixed(3)} > 0.5 (likely a polygon)`,
      );
      return null;
    }
    ...
    if (!stroke.isSmooth && analysis.dominantDirection !== 'circular' && analysis.symmetry < 0.4) {
      console.log(
        `Ellipse: REJECT - rough (${stroke.isSmooth}) AND not circular (${analysis.dominantDirection}) AND low symmetry (${analysis.symmetry.toFixed(3)})`,
      );
    ...
    const maxAllowedCorners =
      stroke.isSmooth && analysis.symmetry > 0.6 ? 40 : stroke.isSmooth ? 15 : 4;
    if (analysis.hasSharpCorners && analysis.cornerCount > maxAllowedCorners) {
      console.log(
        `Ellipse: REJECT - too many corners: ${analysis.cornerCount} > ${maxAllowedCorners} (smooth: ${stroke.isSmooth})`,
      );
      return null;
    }
    ...
    if (normalizedError > thresholds.ellipseMaxError) {
      console.log(
        `Ellipse: REJECT - error too high: ${normalizedError.toFixed(3)} > ${thresholds.ellipseMaxError}`,
      );
      return null;
    }
    ...
    console.log(
      `Ellipse: Shape determination - radiusX=${ellipseFit.radiusX.toFixed(1)}, radiusY=${ellipseFit.radiusY.toFixed(1)}, eccentricity=${ellipseFit.eccentricity.toFixed(3)}, isCircle=${isCircle}`,
    );

    // Check eccentricity for ellipse
    if (!isCircle && ellipseFit.eccentricity < thresholds.ellipseMinEccentricity) {
      console.log(
        `Ellipse: REJECT - too close to circle: eccentricity ${ellipseFit.eccentricity.toFixed(3)} < ${thresholds.ellipseMinEccentricity}`,
      );
    ...
    const aspectRatio =
      Math.max(ellipseFit.radiusX, ellipseFit.radiusY) /
      Math.min(ellipseFit.radiusX, ellipseFit.radiusY);
    ...
    if (confidence < thresholds.minConfidence) {
      console.log(
        `Ellipse: REJECT - low confidence: ${confidence.toFixed(3)} < ${thresholds.minConfidence}`,
      );
      return null;
    }

    console.log(
      `Ellipse: SUCCESS - ${shapeType} detected with confidence ${confidence.toFixed(3)}, error ${normalizedError.toFixed(3)}`,
    );
  ...
  private fitBoundingBoxEllipse(bbox: BoundingBox): EllipseFit {
    ...
    const eccentricity =
      radiusX !== radiusY
        ? Math.sqrt(1 - Math.min(radiusX, radiusY) ** 2 / Math.max(radiusX, radiusY) ** 2)
        : 0;
  ...
  private isCircular(ellipse: EllipseFit, tolerance: number): boolean {
    const aspectRatio =
      Math.max(ellipse.radiusX, ellipse.radiusY) / Math.min(ellipse.radiusX, ellipse.radiusY);
