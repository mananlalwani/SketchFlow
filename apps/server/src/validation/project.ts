...
function hasSafeDepthAndFields(value: unknown, depth = 0): boolean {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (key === 'imageData' && typeof child === 'string' && child.length > MAX_IMAGE_DATA_CHARS) return false;
...
export const collaboratorUserIdSchema = z.string().regex(/^user_[A-Za-z0-9]+$/).max(128);
