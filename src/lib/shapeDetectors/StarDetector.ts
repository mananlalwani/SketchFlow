...
export class StarDetector implements ShapeDetector {
  ...
  private findStarPattern(points: Point[], bbox: { centerX: number; centerY: number; width: number; height: number }): StarCandidate | null {
    ...
    const regularity = this.calculateStarRegularity(points, peaks, valleys, center, outerRadius, innerRadius);
    if (regularity < 0.5) {
      return null;
    }
    ...
    const confidence = this.calculateStarConfidence(regularity, angularRegularity, radiusRatio, peaks.length);
