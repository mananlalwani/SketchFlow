...
export class RectangleDetector implements ShapeDetector {
  ...
  detect(stroke: ProcessedStroke, thresholds: DetectionThresholds): DetectionResult | null {
    ...
    console.log(
      `Rectangle: Analysis - corners: ${analysis.cornerCount}, hasSharp: ${analysis.hasSharpCorners}, complexity: ${stroke.complexity.toFixed(3)}, straightRatio: ${analysis.straightSegmentRatio.toFixed(3)}`,
    );
    ...
    if (requiresCorners) {
      ...
      if (!analysis.hasSharpCorners || analysis.cornerCount < 3 || analysis.cornerCount > 6) {
        console.log(
          `Rectangle: REJECT - corner requirements not met: hasSharp=${analysis.hasSharpCorners}, count=${analysis.cornerCount} (required for straightRatio=${analysis.straightSegmentRatio.toFixed(3)} < 0.55)`,
        );
        return null;
      }
    } else {
      ...
      console.log(
        `Rectangle: ACCEPT corners check - hasSharp=${analysis.hasSharpCorners}, count=${analysis.cornerCount}, requiresCorners=${requiresCorners}`,
      );
    }

    // Should have significant straight segments - focus on ratio rather than the hasLongStraightSegments flag
    const requiredEdgeRatio =
      stroke.complexity > 0.1 ? thresholds.rectangleEdgeRatio : thresholds.rectangleEdgeRatio * 0.7;
    if (analysis.straightSegmentRatio < requiredEdgeRatio) {
      console.log(
        `Rectangle: REJECT - insufficient straight segments: ratio=${analysis.straightSegmentRatio.toFixed(3)} < ${requiredEdgeRatio.toFixed(3)} (hasLong=${analysis.hasLongStraightSegments})`,
      );
      return null;
    }
    console.log(
      `Rectangle: ACCEPT straight segments - ratio=${analysis.straightSegmentRatio.toFixed(3)} >= ${requiredEdgeRatio.toFixed(3)}`,
    );
    ...
    console.log(
      `Rectangle: Found fit with error ${error.toFixed(3)}, normalized ${normalizedError.toFixed(3)}`,
    );

    if (normalizedError > thresholds.rectangleMaxError) {
      console.log(
        `Rectangle: REJECT - error too high: ${normalizedError.toFixed(3)} > ${thresholds.rectangleMaxError}`,
      );
      return null;
    }
    ...
    const confidence =
      (edgeConfidence + cornerConfidence + errorConfidence + symmetryConfidence) / 4;

    console.log(
      `Rectangle: Confidence calculation - edge=${edgeConfidence.toFixed(3)}, corner=${cornerConfidence.toFixed(3)}, error=${errorConfidence.toFixed(3)}, symmetry=${symmetryConfidence.toFixed(3)}, total=${confidence.toFixed(3)}`,
    );

    if (confidence < thresholds.minConfidence) {
      console.log(
        `Rectangle: REJECT - confidence too low: ${confidence.toFixed(3)} < ${thresholds.minConfidence}`,
      );
      return null;
    }

    console.log(
      `Rectangle: SUCCESS - rectangle detected with confidence ${confidence.toFixed(3)}, error ${normalizedError.toFixed(3)}`,
    );
  ...
  private findMostPerpendicularEdges(
    edges: EdgeSegment[],
  ): { edge1: EdgeSegment; edge2: EdgeSegment } | null {
  ...
  private constructRectangleFromEdges(
    points: Point[],
    edge1: EdgeSegment,
    edge2: EdgeSegment,
  ): RectangleFit | null {
