...
export function fitParabola(points: Point[]): ParabolaEquation | null {
  ...
  if (isMoreVertical) {
    ...
    const det =
      n * (sumY2 * sumY4 - sumY3 * sumY3) -
      sumY * (sumY * sumY4 - sumY2 * sumY3) +
      sumY2 * (sumY * sumY3 - sumY2 * sumY2);

    if (Math.abs(det) < 1e-10) return null;

    const detA =
      sumX * (sumY2 * sumY4 - sumY3 * sumY3) -
      sumYX * (sumY * sumY4 - sumY2 * sumY3) +
      sumY2X * (sumY * sumY3 - sumY2 * sumY2);
    const detB =
      n * (sumYX * sumY4 - sumY2X * sumY3) -
      sumX * (sumY * sumY4 - sumY2 * sumY3) +
      sumY2 * (sumY * sumY2X - sumY2 * sumYX);
    const detC =
      n * (sumY2 * sumY2X - sumY3 * sumYX) -
      sumY * (sumY * sumY2X - sumY2 * sumYX) +
      sumX * (sumY * sumY3 - sumY2 * sumY2);
