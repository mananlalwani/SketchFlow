...
export class TriangleDetector implements ShapeDetector {
  ...
  detect(stroke: ProcessedStroke, thresholds: DetectionThresholds): DetectionResult | null {
    ...
    console.log(
      `Triangle: Analysis - corners: ${analysis.cornerCount}, hasSharp: ${analysis.hasSharpCorners}, straightRatio: ${analysis.straightSegmentRatio.toFixed(3)}`,
    );
    ...
    if (requiresCorners && (!analysis.hasSharpCorners || analysis.cornerCount < 2)) {
      console.log(
        `Triangle: REJECT - corner requirements not met: hasSharp=${analysis.hasSharpCorners}, count=${analysis.cornerCount} (required for straightRatio=${analysis.straightSegmentRatio.toFixed(3)} < 0.55)`,
      );
      return null;
    }
    ...
    if (analysis.cornerCount > maxCorners) {
      console.log(
        `Triangle: REJECT - too many corners: count=${analysis.cornerCount} > ${maxCorners}`,
      );
      return null;
    }

    console.log(
      `Triangle: ACCEPT corners check - hasSharp=${analysis.hasSharpCorners}, count=${analysis.cornerCount}, requiresCorners=${requiresCorners}`,
    );

    // Should have significant straight segments
    if (
      !analysis.hasLongStraightSegments ||
      analysis.straightSegmentRatio < thresholds.triangleEdgeRatio
    ) {
      console.log(
        `Triangle: REJECT - insufficient straight segments: hasLong=${analysis.hasLongStraightSegments}, ratio=${analysis.straightSegmentRatio.toFixed(3)} < ${thresholds.triangleEdgeRatio}`,
      );
      return null;
    }
    ...
    if (normalizedError > thresholds.triangleMaxError) {
      console.log(
        `Triangle: REJECT - error too high: ${normalizedError.toFixed(3)} > ${thresholds.triangleMaxError}`,
      );
      return null;
    }
    ...
    if (confidence < thresholds.minConfidence) {
      console.log(
        `Triangle: REJECT - confidence too low: ${confidence.toFixed(3)} < ${thresholds.minConfidence}`,
      );
      return null;
    }

    console.log(
      `Triangle: SUCCESS - triangle detected with confidence ${confidence.toFixed(3)}, error ${normalizedError.toFixed(3)}`,
    );

    // Create the detected shape
    const shape = createDetectedShape(
      'triangle',
      this.getTriangleBoundingBox(triangleFit.vertices),
      {
  ...
  private findBestTriangleFit(points: Point[], analysis: any): TriangleFit | null {
    ...
    // Choose the best fit
    const candidates = [cornerBased, distanceBased, convexHullBased, rightTriangleBased].filter(
      (fit) => fit !== null,
    );
    ...
    for (let i = 0; i < maxCandidates - 2; i++) {
      for (let j = i + 1; j < maxCandidates - 1; j++) {
        for (let k = j + 1; k < maxCandidates; k++) {
          ...
          if (score > bestScore && minDeviation < Math.PI / 6) {
            // Within 30 degrees of 90°
            bestScore = score;
            bestFit = fit;
          }
        }
  ...
  private createTriangleFit(vertices: Point[]): TriangleFit | null {
    ...
    const edges = [
      {
        start: orderedVertices[0],
        end: orderedVertices[1],
        length: distance(orderedVertices[0], orderedVertices[1]),
      },
      {
        start: orderedVertices[1],
        end: orderedVertices[2],
        length: distance(orderedVertices[1], orderedVertices[2]),
      },
      {
        start: orderedVertices[2],
        end: orderedVertices[0],
        length: distance(orderedVertices[2], orderedVertices[0]),
      },
    ];
    ...
    do {
      ...
      for (const point of points) {
        ...
        if (
          next === current ||
          cross > 0 ||
          (cross === 0 && distance(current, point) > distance(current, next))
        ) {
          next = point;
        }
      }
