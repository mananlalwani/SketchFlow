...
export class LineDetector implements ShapeDetector {
  ...
  detect(stroke: ProcessedStroke, thresholds: DetectionThresholds): DetectionResult | null {
    ...
    if (stroke.boundingBox.width < thresholds.lineMinLength && stroke.boundingBox.height < thresholds.lineMinLength) {
      console.log(`Line: REJECT - too short: width=${stroke.boundingBox.width.toFixed(1)}, height=${stroke.boundingBox.height.toFixed(1)} < ${thresholds.lineMinLength}`);
      return null;
    }
    ...
    console.log(`Line: Fitted - error=${normalizedError.toFixed(3)}, isVertical=${lineEquation.isVertical}, slope=${lineEquation.slope?.toFixed(3) || 'N/A'}`);
    
    // Check if error is acceptable
    if (normalizedError > thresholds.lineMaxError) {
      console.log(`Line: REJECT - error too high: ${normalizedError.toFixed(3)} > ${thresholds.lineMaxError}`);
      return null;
    }
    ...
    if (stroke.complexity > thresholds.lineMaxComplexity) {
      console.log(`Line: REJECT - complexity too high: ${stroke.complexity.toFixed(3)} > ${thresholds.lineMaxComplexity}`);
      return null;
    }
    if (strokeAnalysis.straightSegmentRatio < thresholds.lineMinStraightSegmentRatio) {
      console.log(`Line: REJECT - straight segment ratio too low: ${strokeAnalysis.straightSegmentRatio.toFixed(3)} < ${thresholds.lineMinStraightSegmentRatio}`);
      return null;
    }

    const straightness = this.calculateStraightness(points);
    if (straightness < thresholds.lineMinStraightness) {
      console.log(`Line: REJECT - not straight enough: ${straightness.toFixed(3)} < ${thresholds.lineMinStraightness}`);
      return null;
    }
    
    // Calculate confidence based on multiple indicators
    const errorConfidence = 1 - Math.min(1, normalizedError / thresholds.lineMaxError);
    const straightSegmentConfidence = Math.min(1, strokeAnalysis.straightSegmentRatio / thresholds.lineMinStraightSegmentRatio);
    ...
    if (confidence < thresholds.minConfidence) {
      console.log(`Line: REJECT - confidence too low: ${confidence.toFixed(3)} < ${thresholds.minConfidence}`);
      return null;
    }
    
    console.log(`Line: SUCCESS - line detected with confidence ${confidence.toFixed(3)}, straightness ${straightness.toFixed(3)}`);
