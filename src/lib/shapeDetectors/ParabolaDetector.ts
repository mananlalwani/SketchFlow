...
export class ParabolaDetector implements ShapeDetector {
  ...
  detect(stroke: ProcessedStroke, thresholds: DetectionThresholds): DetectionResult | null {
    ...
    console.log(`Parabola: Initial analysis - complexity=${stroke.complexity.toFixed(3)}, smooth=${stroke.isSmooth}, corners=${analysis.cornerCount}, direction=${analysis.dominantDirection}`);
    
    // Should have some curvature (parabolas are curved, not straight)
    if (stroke.complexity < thresholds.parabolaMinCurvature) {
      console.log(`Parabola: REJECT - low curvature: ${stroke.complexity.toFixed(3)} < ${thresholds.parabolaMinCurvature}`);
      return null;
    }
    
    // Parabolas shouldn't be too circular
    if (analysis.dominantDirection === 'circular' && stroke.complexity > 0.3) {
      console.log(`Parabola: REJECT - too circular for parabola (direction=${analysis.dominantDirection}, complexity=${stroke.complexity.toFixed(3)})`);
      return null;
    }
    ...
    console.log(`Parabola: Fitted ${parabolaFit.isVertical ? 'vertical' : 'horizontal'} parabola: a=${parabolaFit.a.toFixed(4)}, b=${parabolaFit.b.toFixed(4)}, c=${parabolaFit.c.toFixed(4)}`);
    ...
    if (Math.abs(parabolaFit.a) < minA) {
      console.log(`Parabola: REJECT - too flat: |a|=${Math.abs(parabolaFit.a).toFixed(6)} < ${minA}`);
      return null;
    }
    ...
    if (Math.abs(parabolaFit.a) > sizeAdjustedMaxA) {
      console.log(`Parabola: REJECT - too steep: |a|=${Math.abs(parabolaFit.a).toFixed(6)} > ${sizeAdjustedMaxA.toFixed(6)}`);
      return null;
    }
    ...
    if (normalizedError > thresholds.parabolaMaxError) {
      console.log(`Parabola: REJECT - error too high: ${normalizedError.toFixed(3)} > ${thresholds.parabolaMaxError}`);
      return null;
    }
    ...
    console.log(`Parabola: Analysis - vertex=(${parabolaInfo.vertex.x.toFixed(1)}, ${parabolaInfo.vertex.y.toFixed(1)}), symmetry=${parabolaInfo.symmetry.toFixed(3)}, direction=${parabolaInfo.openDirection}`);
    
    // Check symmetry
    if (parabolaInfo.symmetry < thresholds.parabolaSymmetryTolerance) {
      console.log(`Parabola: REJECT - low symmetry: ${parabolaInfo.symmetry.toFixed(3)} < ${thresholds.parabolaSymmetryTolerance}`);
      return null;
    }
    ...
    const confidence = (curvatureConfidence + smoothnessConfidence + errorConfidence + symmetryConfidence + openConfidence) / 5;
    
    if (confidence < thresholds.minConfidence) {
      console.log(`Parabola: REJECT - low confidence: ${confidence.toFixed(3)} < ${thresholds.minConfidence}`);
      return null;
    }
    
    console.log(`Parabola: SUCCESS - parabola detected with confidence ${confidence.toFixed(3)}, error ${normalizedError.toFixed(3)}`);
  ...
  private analyzeParabola(
    ...
    if (vertex.x < bbox.minX - bbox.width * 0.5 || vertex.x > bbox.maxX + bbox.width * 0.5 ||
        vertex.y < bbox.minY - bbox.height * 0.5 || vertex.y > bbox.maxY + bbox.height * 0.5) {
