...
import { ShapeDetector, DetectionResult, DetectionThresholds, DEFAULT_THRESHOLDS } from './types';
...
export class ShapeDetectionPipeline {
  ...
  detectShape(points: Point[], options: DetectionOptions = {}): ShapeDetectionResult {
    ...
    if (options.debugMode) {
      console.log(
        `Input: ${points.length} points, processed: ${processedStroke?.processedPoints.length || 0} points`,
      );
      if (processedStroke) {
        console.log(
          `Stroke analysis: closed=${processedStroke.isClosed}, smooth=${processedStroke.isSmooth}, complexity=${processedStroke.complexity.toFixed(3)}`,
        );
      }
    }
      ...
      try {
        ...
        if (result && result.confidence >= finalThresholds.minConfidence) {
          ...
          if (options.debugMode) {
            ...
            console.log(
              `${detector.shapeType} detector: confidence=${result.confidence.toFixed(3)}, error=${result.error.toFixed(3)}, composite=${compositeScore.toFixed(3)}`,
            );
          }
        } else if (options.debugMode && result) {
          console.log(
            `${detector.shapeType} detector: REJECTED - confidence=${result.confidence.toFixed(3)} < ${finalThresholds.minConfidence}`,
          );
    ...
    if (options.debugMode) {
      ...
      console.log(
        `Found ${candidates.length} candidates, best: ${bestCandidate?.shape.type || 'none'}`,
      );
  ...
  private selectBestCandidate(
    ...
    const parabolaEntry = scoredCandidates.find(
      (entry) => entry.candidate.shape.type === 'parabola',
    );
    ...
    if (lineEntry && parabolaEntry) {
      ...
      const lineComplexity =
        typeof lineMetadata.complexity === 'number' ? lineMetadata.complexity : null;
      const parabolaAnalysis = parabolaMetadata.analysis || {};
      const parabolaStraightSegmentRatio = parabolaAnalysis.straightSegmentRatio;
      const parabolaSymmetry =
        parabolaMetadata.parabolaInfo?.symmetry ?? parabolaAnalysis.symmetry ?? 0;
      const parabolaCurvature =
        parabolaMetadata.parabolaInfo?.curvature ?? parabolaMetadata.parabolaFit?.a ?? null;

      let preferParabola = false;

      if (
        typeof lineStraightSegmentRatio === 'number' &&
        lineStraightSegmentRatio < thresholds.lineMinStraightSegmentRatio + 0.05
      ) {
        preferParabola = true;
      }

      if (
        typeof lineComplexity === 'number' &&
        lineComplexity > thresholds.lineMaxComplexity * 0.8
      ) {
        preferParabola = true;
      }

      if (
        typeof parabolaStraightSegmentRatio === 'number' &&
        parabolaStraightSegmentRatio < thresholds.lineMinStraightSegmentRatio * 0.9
      ) {
        preferParabola = true;
      }
    ...
    const highPriorityCandidate = scoredCandidates.find(
      (c) =>
    ...
    if (highPriorityCandidate) {
      console.log(
        `Selecting higher priority ${highPriorityCandidate.candidate.shape.type} (priority ${highPriorityCandidate.priority}) over ${best.candidate.shape.type} (priority ${best.priority})`,
      );
  ...
  setEnabledDetectors(detectorTypes: string[]): void {
    ...
    this.detectors = allDetectors.filter((detector) => detectorTypes.includes(detector.shapeType));
