...
export function ProjectShareDialog({ project, onUpdate, triggerClassName, open: controlledOpen, onOpenChange }: ProjectShareDialogProps) {
  ...
  const [collaborators, setCollaborators] = useState<{ userId: string; email?: string; role: string }[]>([]);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {!isControlled && (
        <DialogTrigger asChild>
          <Button 
            variant="ghost" 
            size="icon" 
            className={triggerClassName || "h-8 w-8 opacity-0 group-hover:opacity-100 transition-opacity"}
      ...
      <DialogContent className="bg-white dark:bg-slate-900 border-slate-200 dark:border-white/10 max-w-md" onClick={(e) => e.stopPropagation()}>
        <DialogHeader>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                {isShared ? <Globe className="w-4 h-4 text-emerald-500 dark:text-emerald-400" /> : <Lock className="w-4 h-4 text-slate-400" />}
                <span className="text-sm font-medium text-slate-900 dark:text-slate-100">Public Link</span>
              </div>
              {isOwner && (
                <Button
                  ...
                  disabled={loading}
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : isShared ? 'Disable' : 'Enable'}
                </Button>
              )}
            </div>
          ...
          {isOwner && (
            <div className="space-y-3 pt-4 border-t border-slate-200 dark:border-white/10">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-slate-900 dark:text-slate-100">Collaborators</span>
              </div>
              ...
              <div className="flex gap-2">
                ...
                <Button variant="default" size="icon" onClick={handleAddCollaborator} disabled={loading || !newCollabEmail.trim()}>
              ...
              {collaborators.length > 0 ? (
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {collaborators.map((c) => (
                    <div key={c.userId} className="flex items-center justify-between bg-slate-100 dark:bg-slate-800/50 rounded px-3 py-2">
                      <div className="flex-1 min-w-0">
                        <span className="text-sm truncate block text-slate-900 dark:text-slate-100">{c.email || 'Unknown user'}</span>
                ...
                <p className="text-xs text-slate-500">No collaborators yet. Add someone by their email address.</p>
              )}
            </div>
          )}
        </div>
